...
from detectors.wrong_side import WrongSideDetector
from detectors.emergency import EmergencyVehicleDetector
from detectors.pothole import PotholeDetector
...
def initialize_detectors():
    """Initialize AI detection models (cached)"""
    try:
        detectors = [
            WrongSideDetector(),
        ...
        st.error(f"Failed to initialize detectors: {e}")
        return []
...
def initialize_processor():
    ...
    if st.session_state.processor is None:
        ...
        st.session_state.detectors_loaded = len(detectors) > 0
...
def render_settings_tab():
    ...
    with col1:
        ...
        st.slider("Confidence Threshold", 0.0, 1.0, settings.CONFIDENCE_THRESHOLD, 0.05)
